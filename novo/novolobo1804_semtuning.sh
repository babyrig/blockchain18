...
then
  ...
  sudo apt update
  sudo apt install -y zip unzip bc curl nano lshw htop vnstat slurm libboost-all-dev libdb5.3++-dev libdb5.3-dev libevent-dev
  cd
  echo ""
...
then
 ...
 RPCPORT=21230
  ...
  echo "maxuploadtarget=300" >> transcendence.conf_TEMP
  echo "bind=" >> transcendence.conf_TEMP
  echo "externalip=" >> transcendence.conf_TEMP
  echo "onlynet=ipv4" >> transcendence.conf_TEMP
